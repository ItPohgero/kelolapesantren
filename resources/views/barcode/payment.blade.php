    ...
    <div class="min-h-screen bg-gray-400 flex justify-center items-center px-2">
        ...
        <div class="p-6 w-max bg-white rounded-2xl shadow-xl z-20">
            ...
            <div class="text-center py-1">
                <p class="text-gray-800 font-bold uppercase">TOKO {{ $toko->nama }}</p>
                <p class="text-xs capitalize">Pemilik : {{ $toko->pemilik }}</p>
            </div>
            <x-alert></x-alert>
            <main class="py-4">
                <div class="py-1 bg-gray-50 rounded-lg">
                    <div class="px-2">
                        ...
                        <!-- loop -->
                        <form action="{{ route('barcode.action.payment', ['pin' => $toko->pin, 'code' => $santri->code]) }}" method="POST" autocomplete="off">
                            @csrf
                            <div class="py-1">
                                <label for="nominal" class="text-xs">Total Pembayaran</label>
                            ...
                            <div class="py-1">
                                <textarea rows="4" name="desc" type="text" class="rounded-lg w-full border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 shadow-sm placeholder-gray-300 text-left text-xs" placeholder="Pembelian untuk">{{ old('desc') }}</textarea>
                            </div>
                                        ...
                                        <div class="relative sm:w-3/4 md:w-1/2 lg:w-1/3 mx-2 sm:mx-auto my-10 opacity-100">
                                            <div class="relative bg-white shadow-lg rounded-lg text-gray-900 z-20" @click.away="showModalPin = false" x-show="showModalPin" x-transition:enter="transition transform duration-300" x-transition:enter-start="scale-0" x-transition:enter-end="scale-100" x-transition:leave="transition transform duration-300" x-transition:leave-start="scale-100" x-transition:leave-end="scale-0">
                                                <header class="flex items-center justify-between p-2">
                                                    <h2 class="font-semibold">Password PIN Customer</h2>
                                                ...
                                                <main class="px-6 mb-6">
                                                    <div class="mt-2">
                                                        <p class="text-xs text-center">Masukkan Password PIN anda disini</p>
