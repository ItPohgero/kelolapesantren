        ...
        <table class="w-full whitespace-no-wrap">
            ...
            <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                ...
                <tr class="text-gray-700 dark:text-gray-400">
                    <td class="px-4 py-3">
                        <a href="{{ route('relasi.anggota', $item->slug) }}" class="flex items-center text-sm">
                    ...
                    <td class="px-4 py-3 text-sm">
                        <div class="flex space-x-3">
                            <a href="#{{ $item->slug }}" rel="modal:open">
                                <div class="w-full inline-flex items-center bg-white leading-none text-red-600 rounded-full p-1 px-2 shadow text-sm capitalize">
                                    <span class="text-xs inline-flex bg-red-600 hover:bg-red-800 text-white rounded-full py-1 px-3 justify-center items-center">Rename</span>
                        ...
                        <div id="{{ $item->slug }}" class="modal shadow-lg">
                            <div class="p-5 bg-gray-50  rounded-xl">
                                <div class="my-2 font-bold text-center uppercase">
                                    Rename Relasi ({{ $item->nama }})
                                </div>
                                <form method="POST" action="{{ route('relasi.update', $item->slug) }}" autocomplete="off">
                                    @csrf
                                    @method('patch')
                                    <input type="hidden" name="old" value="{{$item->nama}}">
                                    <div class="my-2">
                    ...
                    <td class="px-4 py-3 text-sm">
                        <span>{{ $item->santris->count() }} <small>santri</small></span>
                    </td>
                    <td class="px-4 py-3 text-sm">
