<x-main-layout>
        ...
        <table class="w-full whitespace-no-wrap">
            ...
            <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                ...
                <tr class="text-gray-700 dark:text-gray-400">
                    <td class="px-4 py-3">
                        <p class="font-semibold">{{ $item->an }}</p>
                    </td>
                    ...
                    <td class="px-4 py-3 text-sm">
                        ...
                        <div id="{{ $item->id }}" class="modal shadow-lg">
                            <div class="p-5 bg-gray-50  rounded-xl">
                                <div class="my-2 font-bold text-center uppercase">
                                    Edit Santri
                                </div>
                                <form method="POST" action="{{ route('tabungan.up', $item->id) }}" autocomplete="off">
                                    @csrf
                                    @method('patch')
                                    <div class="my-2">
                                        <x-jet-label for="an" value="{{ __('Atas Nama') }}" />
                                        <x-jet-input id="an" class="block my-1 w-full" type="text" name="an" :value="old('an') ?? $item->an" required autofocus />
                                        <x-error for="an"></x-error>
                                    </div>
                                    <div class="my-2">
                                        <x-jet-label for="phone" value="{{ __('Phone') }}" />
                                        <x-jet-input id="phone" class="block my-1 w-full" type="text" name="phone" :value="old('phone') ?? $item->phone" required autofocus />
                                        <x-error for="phone"></x-error>
                                    </div>
                                    <div class="my-2">
                                        <x-jet-label for="password" value="{{ __('New Password') }}" />
                                        <x-jet-input id="password" class="block my-1 w-full" type="text" name="password" required autofocus />
    ...
    <div x-show="isModalOpen" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 z-30 flex items-end bg-black bg-opacity-50 sm:items-center sm:justify-center">
        ...
        <div x-show="isModalOpen" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0 transform translate-y-1/2" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0  transform translate-y-1/2" @click.away="closeModal" @keydown.escape="closeModal" class="w-full px-6 py-4 overflow-hidden bg-white rounded-t-lg dark:bg-gray-800 sm:rounded-lg sm:m-4 sm:max-w-xl" role="dialog" id="modal">
            ...
            <!-- Modal body -->
            <form method="POST" action="{{ route('tabungan.add') }}" autocomplete="off">
                @csrf
                <div class="mt-4 mb-6">
                    <div class="my-2">
                        <x-jet-label for="an" value="{{ __('Atas Nama') }}" />
                        <x-jet-input id="an" class="block my-1 w-full" type="text" name="an" :value="old('an')" required autofocus />
                        <x-error for="an"></x-error>
                    </div>
                    <div class="my-2">
                        <x-jet-label for="phone" value="{{ __('Phone') }}" />
                        <x-jet-input id="phone" class="block my-1 w-full" type="text" name="phone" :value="old('phone')" required autofocus />
                        <x-error for="phone"></x-error>
                    </div>
                    <div class="my-2">
                        <x-jet-label for="code" value="{{ __('Code') }}" />
                        <small>Perhatian! akan digenerate sekali, itu artinya tidak bisa diedit dikemudian hari!</small>
                    ...
                    <div class="my-2">
                        ...
                        <small>Perhatian! gunakan password yang beragam karakter untuk memberikan status strong!</small>
                        <x-jet-input id="password" class="block my-1 w-full" type="text" name="password" :value="old('password')" required autofocus />
                        <x-error for="password"></x-error>
                    </div>
                </div>
