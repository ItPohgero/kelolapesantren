<x-main-layout>
    <x-search></x-search>
    <main class="bg-white dark:bg-gray-800 rounded-xl shadow p-5">
        <p class="font-bold">Profile Pondok Pesantren Masih Kosong</p>
        <small>Isikan dengan baik dan benar</small>
        <form method="POST" action="{{ route('pondok.store') }}" autocomplete="off">
            @csrf
            <div class="my-2">
                <x-jet-label for="nama" value="{{ __('Nama Pesantren') }}" />
            ...
            <div class="my-2">
                ...
                <x-jet-input id="pengasuh" class="block my-1 w-full" type="text" name="pengasuh" :value="old('pengasuh')" required autofocus />
